function dets = detect_positive(imfile, bbox, model, overlap, views)
im = color(imread(imfile));
[im, bbox, ratio, offset] = cropnscalepos(im, bbox, model);

pyra = featpyramid(im, model);
dets = gpdetect(pyra, model, -2, bbox, overlap, views);
% dets = zeros(0, 6);
% for j = 1:length(views)
%     [det, bs] = gpdetect(pyra, model, -2, bbox, overlap, views(j));
%     if(~isempty(det))
%         dets(end+1:end+size(det,1), :) = det;
%     end
% end
dets(:, 1:4) = dets(:, 1:4)./ ratio;
dets(:, [1 3]) = dets(:, [1 3]) + offset(1);
dets(:, [2 4]) = dets(:, [2 4]) + offset(2);

function [im, box, ratio, offset] = cropnscalepos(im, box, model)
...
%padx = pad;
%pady = pad;
padx = 0.2*(box(3)-box(1)+1);
pady = 0.2*(box(4)-box(2)+1);
% x1 = max(1, round(box(1) - padx));
% y1 = max(1, round(box(2) - pady));
% x2 = min(size(im, 2), round(box(3) + padx));
% y2 = min(size(im, 1), round(box(4) + pady));

x1 = round(box(1) - padx); y1 = round(box(2) - pady);
x2 = round(box(3) + padx); y2 = round(box(4) + pady);
[im, os] = padimage(im, [x1 y1 x2 y2]);
offset = [x1, y1];
% offset = offset + [x1; y1];

im = im((y1:y2) + os(2), (x1:x2) + os(1), :);
box([1 3]) = box([1 3]) - offset(1) + 1;
box([2 4]) = box([2 4]) - offset(2) + 1;

ratio = 1;
% fast computation.
% maxsz = 2 * max(model.maxsize) * model.sbin;
% ratio = maxsz / max(size(im));
% if(ratio <= .1)
...
function [dets] = gpdetect(pyra, model, thresh, bbox, overlap, views)
...
det = cell(length(views), 1);
parfor i = 1:length(views)
    det{i} = detectOneView(pyra, model, thresh, bbox, overlap, views(i), latent);
end

dets = zeros(0, 6);
for i = 1:length(det)
    if(~isempty(det{i}))
        dets(end+1:end+size(det{i},1), :) = det{i};
    end
end

function [dets, boxes, info] = detectOneView(pyra, model, thresh, bbox, overlap, view, latent)
...
for s = L
  ...
  model = symbol_score(model, s, latent, pyra, bbox, overlap, view);
end
...
if latent 
    if ~isempty(dets)
        ...
        if boxoverlap(clipdets, bbox) < overlap
            disp('overlap requirement failed');
            clipdets(1:4) = bbox; clipdets(end) = -4;
        end
    else
        clipdets(1:4) = bbox; clipdets(end) = -4;
    end
...
function model = symbol_score(model, s, latent, pyra, bbox, overlap, view)
% model  object model
% s      grammar symbol
