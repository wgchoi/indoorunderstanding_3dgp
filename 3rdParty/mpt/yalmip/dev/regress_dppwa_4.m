...
for k = N-1:-1:1
    ...
    % PWA Dynamics, noise 1
    F = F + set(implies(d(1),x1{k+1} == (A*x{k}+B*u{k}+[-0.001;0])));
    F = F + set(implies(d(2),x1{k+1} == (A*x{k}+pi*B*u{k}+[-0.001;0])));
    
    % PWA Dynamics, noise 2
    F = F + set(implies(d(1),x2{k+1} == (A*x{k}+B*u{k}+[0.001;0])));
    F = F + set(implies(d(2),x2{k+1} == (A*x{k}+pi*B*u{k}+[0.001;0])));    
    ...
    if k<N-1
        ...
        J1{k+1} = pwf(mpsol{k+1},x1{k+1},'convexoverlapping');
        J2{k+1} = pwf(mpsol{k+1},x2{k+1},'convexoverlapping');
        sdpvar v
        F = F + set(J1{k+1} < v) + set(J2{k+1} < v);
        obj = sum(t) + v;
    else
        %J1{N} = 0;
        %J2{N} = 0;
