...
parametric = [t1;t2];
t = [t1;t2];
[sol,dgn,Z,J] = mimpmilp(F,obj,sdpsettings('debug',1),parametric)
...
obj = -2*x1-x2+y1+y2;
sol = solvemp(F,obj,[],t);
sol = mpt_removeOverlaps(sol);

sold = dua_test(F,obj,[],t,y)
sold = mpt_removeOverlaps(sold);
...
sold = dua_test(F,obj,[],[t1;t2],[y1;y2]);
sold = mpt_removeOverlaps(sold);
...
sold = dua_test(F,obj,[],[t1;t2;t3],[y1;y2]);
sold = mpt_removeOverlaps(sold);
...
sola = solvemp(F,obj,sdpsettings('mp.al',2),[t1;t2;t3;t4]);
tic
sola = mpt_removeOverlaps(sola);
toc


sold = dua_test(F,obj,[],[t1;t2;t3;t4])
tic
sold = mpt_removeOverlaps(sold);
toc

mpt_ispWAbigger(sold,sol)

mpt_ispWAbigger(sola,sol)
