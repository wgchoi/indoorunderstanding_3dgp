...
%         a(nj) = 1;
%         p_lp.K.l = p_lp.K.l + 1;
%         p_lp.F_struc = [p_lp.F_struc;b a];
%         
%         
%         res = p_lp.F_struc*[1;x];
%         mmm(end+1,length(res))=0;
%         mmm(end,abs(res)>1) = 1;
%         spy(mmm);drawnow
%         if size(mmm,1)>16
%             delete = [];
%             for j = 1:size(mmm,2)
%                 if all(mmm(end-15:end,j)==1)
%                     delete(j)=1;
%                 else
%                     delete(j)=0;
%                 end
...
%             delete = find(delete);
%             if length(delete)>0
% %                 pool = [pool;p_lp.F_struc(delete,:)];
% %                 p_lp.F_struc(delete,:)=[];
% %                 p_lp.K.l = p_lp.K.l-length(delete);
% %                 mmm(:,delete)=[];
%             end
%         end
%         
