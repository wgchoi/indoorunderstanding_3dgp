...
anno_vp_base = './dataset/cvpr13data/annotations_vp/';
...
%% keep only bedroom images
datafiles = remove_image_class(datafiles, preprocess_dir);
...
layout_option = 'sp10_estvp';
% layout_option = 'sp10_gtvp';
% layout_option = 'sp20_hedau';
% layout_option = 'sp20_estvp';
% layout_option = 'sp20_gtvp';

layoutsets = load_layout_data(layout_option);
...
for idx = 1:csize:length(datafiles)
    ...
    for i = 1:setsize
        ...
        tdata(i) = load_gt_vp(tdata(i), fullfile(anno_vp_base, datafiles(idx+i-1).cname, [datafiles(idx+i-1).fname '_vp.mat']));
        tdata(i) = compute_gt_obj3dloc(tdata(i), fullfile(anno_vp_base, datafiles(idx+i-1).cname, [datafiles(idx+i-1).fname '_vp.mat']));
    end
    tdata(setsize+1:end) = [];
    
    % replace layout
    tdata = replace_layout_3d_2(tdata, params2.model, layoutsets);
...
%     disp(i);
%     tdata = load(fullfile(preprocess_dir, datafiles(i).name));
%     tdata = compute_gt_obj3dloc(tdata, fullfile(anno_vp_base, datafiles(i).cname, [datafiles(i).fname '_vp.mat']));
%     annos{i}.hobjs     = tdata.anno.hobjs;
%     annos{i}.camheight = tdata.anno.camheight;
%     annos{i}.alpha     = tdata.anno.alpha;
% end
